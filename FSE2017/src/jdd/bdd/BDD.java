...
public class BDD extends NodeTable {
	...
	public long satCount(int bdd) {
		...
		return (long)(Math.pow(2, getVar(bdd)) * satCount_rec(bdd));
	}

	protected long satCount_rec(int bdd) {
		if(bdd < 2) return bdd;

		if(sat_cache.lookup(bdd)) return  (long)sat_cache.answer;
		int hash = sat_cache.hash_value;

		int low = getLow(bdd);
		int high = getHigh(bdd);

		long ret = (satCount_rec(low) * (long)Math.pow(2, getVar(low)  - getVar(bdd)  -1)) +
				(satCount_rec(high) * (long) Math.pow(2, getVar(high) - getVar(bdd)  -1));
	...
	//@jeho hack
	/** printthis BDD to console (prints internal structure) */
	public ArrayList<int[]> getTree(int bdd) {return BDDPrinter.getTree(bdd, this);	}
	
	//@jeho hack
	/**  possibly using dont-cares, print satisfying assignment od this BDD */
	public byte[][] getSet(int bdd) {return BDDPrinter.getSet(bdd, num_vars, this, null);	}
	/**  possibly using dont-cares, print satisfying assignment od this BDD */
	public byte[][] getSet(int bdd, int max) {return BDDPrinter.getSet(bdd, max, this, null);	}
	public byte[][] getSet_inv(int bdd, int max) {return BDDPrinter.getSet_inv(bdd, max, this);	}
