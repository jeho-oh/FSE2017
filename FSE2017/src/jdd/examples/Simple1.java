...
public class Simple1 {
	...
	public static void main(String [] args) {
		...
		Integer v1 = bdd.createVar();
		Integer v2 = bdd.createVar();
		Integer v3 = bdd.createVar();
		Integer v4 = bdd.createVar();
		//Integer v5 = bdd.createVar();
		...
		tmp1 = bdd.biimp(v1, v2);	// first we create (v1 AND v2)
		bdd.ref(tmp1);								// Now, we must call BDD.ref() as soon as we have one of these.

		tmp2 = bdd.biimp(v3, v4);
		bdd.ref(tmp2);
		
		int f1 = bdd.or(tmp1, tmp2);  // This is equal to (v1 AND v2) OR v3.
...
//		tmp2 = bdd.ref(bdd.not(v2));
//		tmp2 = bdd.andTo(tmp2,v3);
//		//tmp2 = bdd.and(tmp2,v4);
//		//Integer f5 = bdd.biimp(v1, tmp2);
//		Integer f4 = bdd.and(tmp2, v1);
		...
		// a table with four columns (internal ID, variable, low child, high child).
